...
import 'package:profile_challenge_app/app/core/config/loyalty_system_config.dart';
...
import 'package:profile_challenge_app/app/features/wallet/service/customer_access_import_service.dart';
import 'package:profile_challenge_app/app/features/wallet/service/recipient_validation_service.dart';
import 'package:profile_challenge_app/app/features/wallet/service/wallet_activation_storage_service.dart';
import 'package:profile_challenge_app/app/features/wallet/service/wallet_balance_service.dart';
import 'package:profile_challenge_app/app/features/wallet/service/wallet_signing_service.dart';
...
class WalletController extends BaseController {
  WalletController({
    ...
    WalletTransactionHistoryService? historyService,
       ...
       _signingService = signingService ?? Get.find<WalletSigningService>(),
       _historyService =
  ...
  Future<void> refreshWalletBalance() async {
    ...
    try {
      currentBalance.value = await _balanceService.fetchBalance(
        config: activeConfig,
        walletAccess: activeAccess,
      );
  ...
  Future<void> unlockWalletForSend() async {
    ...
    try {
      ...
      showMessage(error.message);
      Get.snackbar(AppString.walletSendFailure, error.message);
    }
  }
  ...
  Future<void> confirmSend() async {
    ...
    if (secret == null || secret.isEmpty) {
      showMessage(AppString.walletLockedMessage);
    ...
    try {
      final result = await _signingService.sendPayment(
        config: activeConfig,
        senderSecret: secret,
        recipientPublicKey: recipient,
        amount: amount,
      ...
      Get.snackbar(AppString.walletSendTitle, AppString.walletSendSuccess);
      await refreshWalletBalance();
      ...
      Get.snackbar(AppString.walletSendFailure, errorMessage);
    } finally {
